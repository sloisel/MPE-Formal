...
hypotheses.  A reader who accepts mathlib can therefore audit the claims from this file
alone.  Each proof body is a one-line reference into the development, where the work
lives.

The bottom of the file checks, on every build, that no unintended axioms are used: each
theorem must rest on nothing beyond `propext`, `Classical.choice` and `Quot.sound`
(Lean's standard foundations) — in particular not on `sorryAx`, the mark of an
incomplete proof.  The same check re-verifies self-containedness: if a name from this
development ever crept into one of the statements, the build would fail.
-/
...
`#print axioms` reports what each theorem rests on; the `run_cmd` below asserts it,
failing the build unless each theorem depends only on `propext`, `Classical.choice` and
`Quot.sound` — so in particular not on `sorryAx` — and unless its statement is free of
every name from this development. -/
